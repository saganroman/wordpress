...
function motopressCERemoveTemporaryPost() {
    ...
    if (!is_null($post)) {
        ...
        if ($delete === false) {
            $errors[] = __("An error occurred while removing temporary post", 'motopress-content-editor');
        }
    }
    ...
    if (!empty($errors)) {
        if (mpceSettings()['debug']) {
            ...
            motopressCESetError(__("An error occurred while removing temporary post", 'motopress-content-editor'));
        }
    }
    exit();
}